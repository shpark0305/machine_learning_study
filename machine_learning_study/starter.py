...
if __name__ == "__main__":
    ...
    # OCSVM의 score_samples도 Isolation Forest와 마찬가지로 "정상일수록 큰 값"을 뱉습니다.
    # 따라서 이상탐지 점수로 쓰기 위해 부호(-)를 똑같이 뒤집어 줍니다.
    val_window_scores  = -model.score_samples(val_X)
    test_window_scores = -model.score_samples(test_X)

    # [우리가 완성했던 AUPR 치트키 후처리] 윈도우 스코어 롤링 필터 (window=3)
    ...
    # window score → timestep score 환산 (이하 기존 코드 동일)
